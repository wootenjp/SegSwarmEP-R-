...
process_ant <- function(
    graph, adj_matrix, num_districts, alpha, beta, rho, df.dist, max_district_size, 
  ...
  for (district_index in 1:num_districts) {
    ...
    # Track cluster size
    cluster_sizes[district_index] <- df.dist$total[initial_node]
    ...
    while (length(eligible_nodes) > 0) {
      ...
      available_neighbors <- available_neighbors[
        (cluster_sizes[district_index] + df.dist$total[available_neighbors]) <= max_district_size
      ]
      ...
      cluster_sizes[district_index] <- cluster_sizes[district_index] + df.dist$total[next_node]  # <-- add this line
  ...
  while (length(unassigned_nodes) > 0 && recycle_count < max_recycles) {
    ...
    for (district_index in 1:num_districts) {
      ...
      # Track cluster size
      cluster_sizes[district_index] <- cluster_sizes[district_index] + df.dist$total[initial_node]
...
aco_districting <- function(
    graph, adj_matrix, num_ants, num_districts, num_iterations,
    df.dist, sf.dist, max_district_size, penalty_contiguity, bonus_contiguity,
    compactness_scale, demographic_scale, size_scale_factor, 
    search_radius,
  ...
  clusterExport(cl, c(
    "df.dist", "sf.dist", "process_ant", "calculate_score", "calculate_polsby_popper", "graph", "adj_matrix",
    "num_districts", "max_district_size", "penalty_contiguity", "bonus_contiguity", "compactness_scale", 
    "demographic_scale", "is_contiguous", "st_area", "st_union", "st_drop_geometry", 
  ...
  while (iteration <= num_iterations) {
    ...
    ant_results <- parLapply(cl, 1:num_ants, function(ant) {
      assignment <- process_ant(graph, adj_matrix, num_districts, alpha, beta, rho, df.dist, max_district_size, 
    ...
    if (length(unique(all_district_data_long$cluster)) == num_districts) {
      ...
      if (length(h.cluster) > 0) {
        h_values[iteration] <- h.cluster[2, 2] 
    ...
    # Regular Pheromone Update: only if we have a valid best_assignment and positive best_score
    if (!is.null(best_assignment) && !all(is.na(best_assignment)) && !is.nan(best_score) && best_score > 0) {
      scaled_score <- log(best_score + 1)
      # update only pairs inside each district (much faster than full matrix loops)
      for (d in seq_len(num_districts)) {
        nodes_in_d <- which(best_assignment == d)
        if (length(nodes_in_d) > 1) {
          pairs <- utils::combn(nodes_in_d, 2)
          # apply updates
    ...
    # Elitist Pheromone Update (only after burn-in period) — per-district pairing
    if (iteration > (burn_in) && length(elite_assignments) > 0) {
      for (idx in seq_along(elite_assignments)) {
        elite_assignment <- elite_assignments[[idx]]
        if (is.null(elite_assignment)) next
        elite_score <- elite_scores[idx]
        scaled_elite_score <- ifelse(is.nan(elite_score) || elite_score <= 0, 0, log(elite_score + 1) * 1.5)
        if (scaled_elite_score == 0) next
        for (d in seq_len(num_districts)) {
          nodes_in_d <- which(elite_assignment == d)
          if (length(nodes_in_d) > 1) {
            pairs <- utils::combn(nodes_in_d, 2)
            for (k in seq_len(ncol(pairs))) {
              i <- pairs[1, k]; j <- pairs[2, k]
